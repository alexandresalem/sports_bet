...
from horse.constants import (BASES_DIR, BBC_DIR, BBC_PICKLE_DIR, BETS_DIR,
                             CONSERVADOR, HISTORICAL_BASES_DIR, MODELOS_DIR,
                             ODDFAIR_DIR)
from horse.utils import (acerto, financial_result_model, logger, place_bet,
                         place_bet_income, prepare_model_dataset)
...
def build_new_model(test=True, clf=None):
    ...
    df = pd.read_csv(historical_filename, low_memory=False,
                     parse_dates=['date'])
        ...
        if pd.isnull(column["lucro_teste"]):
            ...
            if test:
                ...
                for year in years[1:]:
                    ...
                    scaled_train_result_path = os.path.join(
                        result_folder, scaled_train_result_name)
                    test_result_path = os.path.join(
                        result_folder, test_result_name)
                    scaled_test_result_path = os.path.join(
                        result_folder, scaled_test_result_name)
                    ...
                    df_train_scaled = pd.DataFrame(
                        X_train_scaled, columns=features_scaled)
                    X_train_export = pd.concat(
                        [df_train, df_train_scaled], axis=1)
                    X_train_export.to_csv(
                        scaled_train_result_path, index=False)
                    ...
                    if 'Classifier' in column['type']:
                        df_test['winner_pred'] = list(
                            clf.predict(X_test_scaled))
                        df_predict = pd.DataFrame(data=clf.predict_proba(X_test_scaled),
                                                  columns=['loose_chance', 'win_chance'])

                        X_test_export = pd.concat(
                            [df_test, df_test_scaled, df_predict], axis=1)
                        X_test_export['place_bet'] = X_test_export.apply(
                            lambda row: place_bet(row['win_chance'], row['odds']), axis=1)
                    else:
                        df_test['income_pred'] = list(
                            clf.predict(X_test_scaled))
                        X_test_export = pd.concat(
                            [df_test, df_test_scaled], axis=1)
                        X_test_export['place_bet'] = X_test_export.apply(lambda row: place_bet_income(row['income_pred']),
                                                                         axis=1)

                    X_test_export['income_bets'] = X_test_export.apply(lambda x: financial_result_model(
                        CONSERVADOR, x['odds'], x['winner'], x['place_bet']), axis=1)
                    X_test_export['acerto'] = X_test_export.apply(
                        lambda x: acerto(x['winner'], x['place_bet']), axis=1)
                    ...
                    df_models.loc[index, f'acertos_teste_{year}'] = round(
                        X_test_export['acerto'].sum()/X_test_export['place_bet'].sum(), 4)
...
def run_new_model(date_string, clf=None, days=1, racing_hours=False):
    ...
    for index, column in df_models.iterrows():
        ...
        if not pd.isnull(column["use"]):
            start = datetime.now()
            logger.info(
                f'Modelagem: Preparando as bases para rodar o {column["name"]} para o dia {date_string}')
            ...
            if len(files_list):
                ...
                if racing_hours or f'base_bbc_{date_string}.pickle' in os.listdir(os.path.join(BASES_DIR, BBC_PICKLE_DIR)):
                    ...
                    df_history = pd.read_pickle(os.path.join(
                        BASES_DIR, BBC_PICKLE_DIR, f'base_bbc_{date_string}.pickle'))
                else:
                    historical_filename = os.path.join(
                        BASES_DIR, f'base_bbc_full.csv')
                    df_history = pd.read_csv(
                        historical_filename, low_memory=False, parse_dates=['date'])
                    df_history, le = prepare_model_dataset(df_history)

                    df_history.to_pickle(os.path.join(
                        BASES_DIR, BBC_PICKLE_DIR, f'base_bbc_{date_string}.pickle'))
                    with open(os.path.join(BASES_DIR, BBC_PICKLE_DIR, f'label_encoder_{date_string}.pickle'), 'wb') as handle:
                        pickle.dump(le, handle)

                df_predict, _ = prepare_model_dataset(
                    df_predict, run=True, le=le)
                logger.info(
                    f'Modelagem: Base do dia {date_string} preparada em {datetime.now() - start} segundos')
                ...
                df_history = df_history[(df_history['date'].dt.year != years[0]) & (
                    df_history['date'].dt.year != years[-1])]
                ...
                df_predict = df_predict[df_predict['started']
                                        <= column['started']]
                # AQUIIIIIIIIII QUE FILTRA
                ...
                df_pred_scaled = pd.DataFrame(
                    X_pred_scaled, columns=features_scaled)
                ...
                try:
                    ...
                    clf = pickle.load(
                        open(os.path.join(MODELOS_DIR, 'novos', f'{column["name"]}_2022.model'), 'rb'))
                    logger.info(
                        f'O {column["name"]} levou {datetime.now() - start} segundos para carregar')

                    logger.info(
                        f'Rodando Modelo: Started Model {column["name"]} Prediction for {date_string}')
                    df_results = pd.DataFrame(data=clf.predict_proba(
                        X_pred_scaled), columns=['loose_chance', 'win_chance'])

                    del clf
                    gc.collect()

                    X_pred_export = pd.concat(
                        [df_predict, df_pred_scaled, df_results], axis=1)
                    X_pred_export['winner_pred'] = X_pred_export.apply(
                        lambda row: place_bet(row['win_chance'], row['odds']), axis=1)
                    ...
                    X_pred_export.drop_duplicates(
                        subset=['date', 'time', 'city', 'horse'], inplace=True, keep='last')
                    X_pred_export.to_csv(filename, index=False)
                    logger.info(
                        f'Rodando Modelo: Apostas do {column["name"]} para o dia {date_string} salvas com sucesso!')
                except Exception as e:
                    logger.warning(e)

            else:
                logger.warning(
                    f'Pre-race: Não existe base para o dia {date_string}')
